...
def RotationOperatorMatrix(Ni, Nj, theta, diskMask=True):
    ...
    for i in range(0, Ni):
        for j in range(0, Nj):
            ...
            if not (diskMask) or ((i - cij) * (i - cij) + (j - cij) * (j - cij) <= (cij + 0.5) * (cij + 0.5)):
                ...
                linij = ToLinearIndex([i, j], [Ni, Nj])
                # The interpolation points
                ijOld = CoordRotationInv([i, j], [Ni, Nj], theta)
                # The indices used for interpolation and their weights
                linIntIndicesAndWeights = LinIntIndicesAndWeights(ijOld, [Ni, Nj])
                ...
                for indexAndWeight in linIntIndicesAndWeights:
                    ...
                    linIndexOld = ToLinearIndex(indexOld, [Ni, Nj])
                    weight = indexAndWeight[2]
                    rotationMatrix[linij, linIndexOld] = weight
    return rotationMatrix


def MultiRotationOperatorMatrix(Ni, Nj, Ntheta, periodicity=2 * np.pi, diskMask=True):
